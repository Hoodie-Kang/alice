...
func NewRefreshService(config *MasterConfig, refreshInput *master.Result, pm types.PeerManager) (*refresh_service, error) {
	s := &refresh_service{
		config: config,
...
func (p *refresh_service) OnStateChanged(oldState types.MainState, newState types.MainState) {
	if newState == types.StateFailed {
		...
		if err == nil {
			WriteMasterResult(p.config, p.refreshInput, result)
		} else {
