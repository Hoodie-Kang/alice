...
const childRefreshProtocol = "/child-refresh/1.0.0"

func Bip32Child(arg string) {
	config, err := child.ReadChildConfigFile(arg)
	if err != nil {
	...
	service, err := child.NewService(config, pm)
	if err != nil {
	...
	pm2 := peer.NewPeerManager(utils.GetPeerIDFromPort(config.Port), host, childRefreshProtocol)
	err = pm2.AddPeers(config.Peers)
	if err != nil {
		log.Crit("Failed to add peers", "err", err)
	}
	// Create a new service.
	refreshService, err := child.NewRefreshService(config, masterResult, pm2)
	if err != nil {
		log.Crit("Failed to new service", "err", err)
	}
