...
//export refreshAndSign
func refreshAndSign(a int64, b int64, message string) {
	port := strconv.FormatInt(a, 10)
	peer := strconv.FormatInt(b, 10)
	dir, err := os.Getwd()
	if err != nil {
		logger.Error("get cwd error", map[string]string{"err": err.Error()})
	}

	path := fmt.Sprintf("%s/storage/emulated/0/android/data/com.example.keygen_app/files/child-Bob.json", dir)
	
	refresh := Refresh(path, port, peer)
